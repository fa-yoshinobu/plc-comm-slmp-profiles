...
- 可否の根拠はすべて 2026-07-03 の実機検証記録（§8）にある。**根拠のない可否を正準JSONに追加しない**こと。追加時は必ず `source` と `evidence`/`note` を伴う。
...
5. **書込ポリシー**: `write_policy` に `read-only` とあるファミリへの書込は strict に関係なく送信前に拒否（既存の S read-only 実装と同じ扱い。iq-f は X も read-only）。
