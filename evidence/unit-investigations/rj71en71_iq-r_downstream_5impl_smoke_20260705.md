# RJ71EN71 iQ-R Downstream 5-Implementation Smoke

Date: 2026-07-05

This is a read-only downstream implementation smoke for an iQ-R PLC reached
through RJ71EN71. RJ71EN71 intentionally uses the base `melsec:iq-r` profile;
this is no-new-profile confirmation evidence, not QJ71E71-100/LJ71E71-100
...
- Target PLC/profile: iQ-R via RJ71EN71 / `melsec:iq-r`
- Endpoint: `192.168.250.100:1025` TCP
- Read target: `D1000`, 1 word
- Intent: read-only
- Purpose: RJ71EN71 downstream smoke with the base iQ-R profile
...
|----------------|-------------|--------|-------------|
| .NET | `dotnet run --project samples/PlcComm.Slmp.Cli -- read-soak ... --plc-profile melsec:iq-r --device D1000 --points 1 --iterations 1 --quiet` | pass | 1 iteration, 0 failures, latency 19 ms |
...
This confirms that all five downstream implementations can read through
RJ71EN71 using `melsec:iq-r`.

The GOAL-3 Ethernet-unit acceptance item remains pending because RJ71EN71 has
no separate new profile and is intentionally verified through the base iQ-R
profile.
