# QCPU via QJ71E71-100 / melsec:qcpu:qj71e71-100 Profile Definition

This is a unit-profile definition backed by live Q12HCPU + QJ71E71-100 evidence.
The profile keeps `melsec:qcpu` as its base profile for device range and address-resolution behavior, but uses the Ethernet-unit communication shape measured through QJ71E71-100.
...
| Area | Base `melsec:qcpu` | Unit result | Decision |
|------|--------------------|-------------|----------|
| Frame | 3E/Q-L baseline | 4E + Q/L works | Add unit profile |
| Type name / block | blocked by base policy | live pass | Unit feature differs |
| Extended routes | not adopted on base | configured `J` and `U\G` pass | Unit feature differs |
| Limits | base Q-series plain limits | unit plain and ext limits measured | Use live unit limits |
| Device ranges | QCPU baseline | same family/range model | Inherit base range data |

## Item

| Item | Value |
|------|-------|
| profile | `melsec:qcpu:qj71e71-100` |
| definition_type | `live` |
| base_profile | `melsec:qcpu` |
| source_evidence | `evidence/unit-investigations/plans/results/qj71e71-100_q12hcpu.json` |
| live_verified | `true` |
| verified_models | `Q12HCPU via QJ71E71-100` |
| scope | `ethernet-unit` |
| frame | `4E` |
| compat | `Q/L` |
...
| random_read_word_ext | 185 |  | 4080 | live |
