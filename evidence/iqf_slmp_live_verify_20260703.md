...
| Extended random word read | ext max 96, over `C054` | limit | `U1\G0` 0080 path: 96 pass; 97 returned `C054` |
| Extended random word write | ext max 80, weighted max 960, over `C054` | limit | `U1\G0` 0080 path: 80 word pass; 81 word returned `C054`; 68 dword pass; 69 dword returned `C054`; readback is not used as a decision condition because the unit may overwrite buffer values |
| Extended random bit write | ext max 94, over `C053` | limit | 0081 path: 94 M-bit reset writes pass; 95 returned `C053` |
...
| Limits | Adopt JSON limits for `melsec:iq-f`; 0080/0081 extended random routes have live evidence for ext-specific 96 / 80 weighted 960 / 94 limits | None |
